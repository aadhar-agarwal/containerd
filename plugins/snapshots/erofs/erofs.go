...
func NewSnapshotter(root string, opts ...Opt) (snapshots.Snapshotter, error) {
	...
	// Check dmverity support if enabled (platform-specific validation)
...
// rootHashPath returns the path to the dm-verity root hash file for a layer
...
func (s *snapshotter) createErofsMount(id string, layerBlob string) mount.Mount {
	...
	// Standard EROFS mount with loop device
	return mount.Mount{
		Source:  layerBlob,
		Type:    "erofs",
...
func (s *snapshotter) mounts(snap storage.Snapshot, _ snapshots.Info) ([]mount.Mount, error) {
	...
	if len(snap.ParentIDs) == 0 {
		if layerBlob, err := s.lowerPath(snap.ID); err == nil {
			...
			return []mount.Mount{
				s.createErofsMount(snap.ID, layerBlob),
			}, nil
		}
...
func (s *snapshotter) Commit(ctx context.Context, name, key string, opts ...snapshots.Opt) error {
	...
	// Format layer blob with dm-verity if enabled
	if s.enableDmverity {
