...
2. The root hash and metadata are stored in a `.dmverity` file alongside the layer blob.

3. When mounting a layer, the EROFS snapshotter creates a dm-verity device using the
   stored root hash, ensuring that any corruption or tampering will be detected at
   read time.
...
If dm-verity is enabled, a `.dmverity` file will also be present:
```
  .erofslayer
  fs
  layer.erofs
